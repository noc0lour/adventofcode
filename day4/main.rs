...
fn main() {
    ...
    for (_, [card_id, winning_numbers, my_numbers]) in line_re.captures_iter(&contents).map(|c| c.extract()){
        let wn: Vec<u32> = winning_numbers.trim().split(" ").filter_map(|n| n.parse::<u32>().ok()).collect();
        let mn: Vec<u32> = my_numbers.trim().split(" ").filter_map(|n| n.parse::<u32>().ok()).collect();
        let number_wins = mn.iter().filter_map(|n| wn.iter().find(|m| *m == n)).count();
    ...
    while card_ids.iter().count() > 0{
        ...
        card_ids = card_ids.iter().map(|c| (c, card_points[*c])).map(|(id, points)| (id+1)..(id+points+1)).collect::<Vec<_>>().iter().flat_map(|it| it.clone()).collect();
